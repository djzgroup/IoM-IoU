...
% % if size(v1.boxes, 1)>=2
% %     boundslast1 = [v1.boxes(end-1,1:2) v1.boxes(end-1,3:4)-v1.boxes(end-1,1:2)+1];
% %     newbounds = getNewBoundingBox(bounds1, boundslast1);
% %     % catch
% %     %     fprintf('catch here')
% %     % end
% %     bounds2 = [v2.boxes(:,1:2) v2.boxes(:,3:4)-v2.boxes(:,1:2)+1];
% %     iou1 = inters_union(bounds1,bounds2);
% %     iou2 = inters_union(newbounds, bounds2);
% %     iou = 0.5*iou1 + 0.5*iou2;
% % else
%     bounds2 = [v2.boxes(:,1:2) v2.boxes(:,3:4)-v2.boxes(:,1:2)+1];
...
% Mov-IoU algorithm by Linchao He
bounds1 = [v1.boxes(end,1:2) v1.boxes(end,3:4)-v1.boxes(end,1:2)+1];
boundslast1 = [v1.boxes(end-1,1:2) v1.boxes(end-1,3:4)-v1.boxes(end-1,1:2)+1];
newbounds = getNewBoundingBox(bounds1, boundslast1);
bounds2 = [v2.boxes(:,1:2) v2.boxes(:,3:4)-v2.boxes(:,1:2)+1];
...
for i = 1 : N2
    ...
    if iou(i)>=iouth
        ...
        elseif strcmp(costtype, 'iouscore')
            score(i) = 0.5*scores2 + 0.5*iou(i);
        end
...
function newbounds = getNewBoundingBox(bounds1, bounds2)
x1 = bounds1(:,1);
y1 = bounds1(:,2);
w1 = bounds1(:,3);
h1 = bounds1(:,4);
...
w2 = bounds2(:,3);
h2 = bounds2(:,4);
newbounds = [2.*x1 - x2, 2.*y1 - y2, (w1 + w2)./2, (h1 + h2)./2];
