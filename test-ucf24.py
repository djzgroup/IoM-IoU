...
parser.add_argument('--data_root', default='K:/Action/data/ucf24/', help='Location of VOC root directory')
parser.add_argument('--save_root', default='K:/Action/data/output/', help='Location to save checkpoint models')
    ...
    for val_itr in range(len(val_data_loader)):
        ...
        for b in range(batch_size):
            ...
            for cl_ind in range(1, num_classes):
                scores = conf_scores[:, cl_ind]
                scores = scores.squeeze()
                c_mask = scores.gt(args.conf_thresh)  # greater than minmum threshold
                scores = scores[c_mask].squeeze()
                # print('scores size',scores.size())
                with torch.no_grad():
                    if scores.dim() == 0 or scores.equal(torch.Tensor([])):
                        # print(len(''), ' dim ==0 ')
